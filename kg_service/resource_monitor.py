#!/usr/bin/env python3
"""
Shared resource monitoring for all KG processes

For batch jobs: use ResourceMonitor as a context manager. It samples CPU/RAM
and Jetson GPU load on a background thread, then writes a Prometheus
textfile-collector-compatible .prom file on exit. Point node_exporter's
...
and start_background_sampler() to continuously update them while the server runs.
"""
import time
import threading
import psutil
...
class ResourceMonitor:
    ...
    def _write_summary(self):
        ...
        duration_gauge.labels(job_name=self.job_name).set(duration)
